...
checkpoint first_3d_filter:
    ...
    output:
        esm_csv = f"{OUTPUT_DIR}/checkpoints/ESM_metrics_combined.csv",
    ...
    run:
        ...
        combined_esm_df.to_csv(output.esm_csv, index=False) # Save the combined DataFrame for transparency and debugging
...
rule plot_gnina:
    ...
    output:
        # Update filenames to reflect GNINA metrics
        caption_file = f"{OUTPUT_DIR}/visualisations/gnina_metrics.rst",
        scatter_plot = report(
            f"{OUTPUT_DIR}/visualisations/gnina_metrics.png",
            caption=f"{OUTPUT_DIR}/visualisations/gnina_metrics.rst",
            category="Visualisations",
            subcategory="3D Metrics Comparison",
            labels={"Figure": "Multi-Model GNINA Metrics"}
        )
    run:
        
        caption_path = f"{OUTPUT_DIR}/visualisations/gnina_metrics.rst"
        plot_path = f"{OUTPUT_DIR}/visualisations/gnina_metrics.png"
        ...
        # Generate the .rst caption file
        caption_text = (
            "Scatter plots showing the distribution of **GNINA CNNscore** vs **GNINA Affinity** "
            "for the ESM model, Second Model (Protein-only), Second Model (Cofold), and Boltz.\n\n"
            "The highlighted red rectangles show the target region of designs with high "
            "predicted binding probability (CNNscore) and strong binding affinity."
        )
...
rule plot_metric_distributions:
    input:
        ...
        second_cofold_csv = rules.gather_final_candidates.output.second_cofold_csv,
        boltz_csv = rules.gather_final_candidates.output.boltz_csv,
        # --- NEW: Boltz native CSV ---
        boltz_native_csv = f"{OUTPUT_DIR}/BOLTZ_native_metrics.csv"
    output:
        # A single shared caption file for all seven plots
        caption_file = f"{OUTPUT_DIR}/visualisations/distributions.rst",
        
        # Original 4 comparative plots
        plddt_plot = report(
            f"{OUTPUT_DIR}/visualisations/dist_pLDDT.png",
            caption=f"{OUTPUT_DIR}/visualisations/distributions.rst",
            category="Visualisations",
            subcategory="Metric Distributions",
            labels={"Metric": "1. pLDDT"}
        ),
        tmscore_plot = report(
            f"{OUTPUT_DIR}/visualisations/dist_TMscore.png",
            caption=f"{OUTPUT_DIR}/visualisations/distributions.rst",
            category="Visualisations",
            subcategory="Metric Distributions",
            labels={"Metric": "2. TMscore"}
        ),
        affinity_plot = report(
            f"{OUTPUT_DIR}/visualisations/dist_Gnina_affinity.png",
            caption=f"{OUTPUT_DIR}/visualisations/distributions.rst",
            category="Visualisations",
            subcategory="Metric Distributions",
            labels={"Metric": "3. GNINA Affinity"}
        ),
        cnnscore_plot = report(
            f"{OUTPUT_DIR}/visualisations/dist_Gnina_CNNscore.png",
            caption=f"{OUTPUT_DIR}/visualisations/distributions.rst",
            category="Visualisations",
            subcategory="Metric Distributions",
            labels={"Metric": "4. GNINA CNNscore"}
        ),
        
        # --- NEW: 3 Boltz Native plots ---
        boltz_conf_plot = report(
            f"{OUTPUT_DIR}/visualisations/dist_Boltz_confidence.png",
            caption=f"{OUTPUT_DIR}/visualisations/distributions.rst",
            category="Visualisations",
            subcategory="Metric Distributions",
            labels={"Metric": "5. Boltz Confidence"}
        ),
        boltz_pred_aff_plot = report(
            f"{OUTPUT_DIR}/visualisations/dist_Boltz_pred_affinity.png",
            caption=f"{OUTPUT_DIR}/visualisations/distributions.rst",
            category="Visualisations",
            subcategory="Metric Distributions",
            labels={"Metric": "6. Boltz Predicted Affinity"}
        ),
        boltz_bin_plot = report(
            f"{OUTPUT_DIR}/visualisations/dist_Boltz_binary_affinity.png",
            caption=f"{OUTPUT_DIR}/visualisations/distributions.rst",
            category="Visualisations",
            subcategory="Metric Distributions",
    ...
    run:
        ...
        # 1. Generate the shared .rst caption file
        caption_text = (
            "KDE Distribution plots for key 3D and binding metrics. "
            "Figures 1-4 demonstrate comparative performance across ESM, Second Model, Cofold, and Boltz. "
            "Figures 5-7 display the internal predictions specific to the Boltz model."
        )
        with open(output.caption_file, "w") as f:
            f.write(caption_text)
            
        # ==========================================
        # PART A: Comparative 4-Model Plots
        # ==========================================
        dfs_compare = [
            pd.read_csv(input.esm_csv),
            pd.read_csv(input.second_csv),
            pd.read_csv(input.second_cofold_csv),
            pd.read_csv(input.boltz_csv)
        ]
        model_names = ["ESM", "Second Model", "Second Model (Cofold)", "Boltz"]
        
        metrics_to_plot_compare = [
            ("pLDDT_mean", output.plddt_plot),
            ("TMscore", output.tmscore_plot),
            ("Gnina_affinity", output.affinity_plot),
            ("Gnina_CNNscore", output.cnnscore_plot)
        ]
        
        for metric_col, out_file in metrics_to_plot_compare:
            # Note: Used the correct parameter names (df_list, columns, df_labels) 
            # to match the function signature you provided
            func.plot_comparative_distributions(
                df_list=dfs_compare,
                columns=[metric_col], 
                df_labels=model_names, 
                output_file=out_file
            )

        # ==========================================
        # PART B: Boltz Native Plots
        # ==========================================
        df_boltz_native = pd.read_csv(input.boltz_native_csv)
        
        metrics_to_plot_boltz = [
            ("confidence_score", output.boltz_conf_plot),
            ("affinity_pred_value", output.boltz_pred_aff_plot),
            ("affinity_probability_binary", output.boltz_bin_plot)
        ]
        
        for metric_col, out_file in metrics_to_plot_boltz:
            # Pass a list containing just the Boltz DF and a single label
            func.plot_comparative_distributions(
                df_list=[df_boltz_native],
                columns=[metric_col], 
                df_labels=["Boltz Native"], 
                output_file=out_file
            )
